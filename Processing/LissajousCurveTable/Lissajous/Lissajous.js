...
function setup() {
  ...
  for(let i = 0 ; i < 7 ; i++){
    circlesX.push(new Circle(baseX+gridScalar*(i+1), baseY, DIAM));
  }
  for(let i = 0 ; i < 7 ; i++){
    circlesY.push(new Circle(baseX, baseY+gridScalar*(i+1), DIAM));
  }
  ...
  for(let i = 0 ; i < 7 ; i++){
    let row = [];
    for(let j = 0 ; j < 7 ; j++){
      row.push(new xDot(dotsX[i].getX(), dotsY[j].getY(), colors[j]));
    }
    xDots.push(row);
  }
